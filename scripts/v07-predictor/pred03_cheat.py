...
class Predictor(nn.Module):
	def __init__(self, reach):
		...
		self.C=self.nnb+1	#append threshold=(min+max)/2 to neighbors
		...
		for idx in range(24):
			kc=idx>>3
			kb=idx&7
			l01=nn.Linear(self.C+kc+kb+1, self.C//2)	#append prev subpixel of current pixel and decoded bits of current subpixel to neighbors
