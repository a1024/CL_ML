...
class Codec(nn.Module):
	def __init__(self):
		...
		self.reach=4
		self.convT=nn.Conv2d(3, 3, (self.reach, self.reach<<1|1), bias=False)
		self.convL=nn.Conv2d(3, 3, (1, self.reach), bias=False)
		#self.conv1C=nn.Conv2d(1, 1, 1, bias=False)
		#self.conv2C=nn.Conv2d(2, 1, 1, bias=False)

		#self.pred0a=nn.Parameter(torch.randn(3, self.reach, self.reach<<1|1))
		#self.pred0b=nn.Parameter(torch.randn(3, self.reach, 1))
		#
		#self.pred1a=nn.Parameter(torch.randn(3, self.reach, self.reach<<1|1))
		#self.pred1b=nn.Parameter(torch.randn(3, self.reach, 1))
		#self.pred1c=nn.Parameter(torch.randn(1, 1, 1))
		#
		#self.pred2a=nn.Parameter(torch.randn(3, self.reach, self.reach<<1|1))
		#self.pred2b=nn.Parameter(torch.randn(3, self.reach, 1))
		#self.pred2c=nn.Parameter(torch.randn(2, 1, 1))
	...
	def forward(self, x):
		...
		pred=self.convT(nn.functional.pad(yuv[:, :, :-1, :], (self.reach, self.reach, self.reach, 0))) + self.convL(nn.functional.pad(yuv[:, :, :, :-1], (self.reach, 0, 0, 0)))#(L, R, T, B)
