...
if __name__ == '__main__':
	...
	lr=0.000500		#always start with high learning rate (0.005 for Adam, 0.1 for SGD), bumping up lr later loses progress
	#lr=0.00001*0.75**6	#C01-L3C
	batch_size=2		#32, <=24, increase batch size instead of decreasing learning rate
	train_crop=1024		#128, 192: batch_size=8
		...
		for name in files:
			...
			try:
				with Image.open(fn) as im:
					...
					if width*height<0x5555555 and ((train_crop!=0 and width>=train_crop and height>=train_crop) or (is_test or train_crop==0)):
						filenames.append(fn)
...
'''
def color_transform_YCbCr_int(x):#YCoCg-R with g & b swapped
...
if __name__=='__main__':
	...
	train_loader=DataLoader(dataset_train, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers)
