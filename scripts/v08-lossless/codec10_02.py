...
class Codec(nn.Module):
	def __init__(self):
		...
		self.reach_p=1		#pixel reach > error/aux reach
		self.reach_e=1
		self.naux=2
		self.blocksize=128	#just to improve train time

		self.ci=2*(self.reach_p+1)*self.reach_p + 2*(self.reach_e+1)*self.reach_e*(self.naux+1)
		self.co=self.naux+1

		self.pred01=Predictor(self.ci, 12, self.co)
		self.pred02=Predictor(self.ci, 24, self.co)#luma predictor is larger
		self.pred03=Predictor(self.ci, 12, self.co)
